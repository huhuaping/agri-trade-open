...
2003-03~2004-12：省份异常，安徽开头，重庆结尾。

2005-01~2005-12：省份异常，上海开头，黑龙江结尾。
- 2005-02 pdf格式异常，无法读取。

2006-01：省份排序异常。上海开头，黑龙江结尾。

2016-09: 进出口表被整合在一块，没法识别。
  ...
  for (j in c(1:12)) {
    ...
    for (k in 1:length(index_target)){
      ...
      tbl_add <- tbl_split  %>%
        ...
        add_column(type=type[k], .before = "V0") %>%
  ...
  for (j in c(1:12)) {
    ...
    # obtain currency unit:
      ...
      tbl_out <- tbl_province %>%
        ...
        mutate(V0=str_extract(V0, "(.+)(?=省|市|自治区)" ),
